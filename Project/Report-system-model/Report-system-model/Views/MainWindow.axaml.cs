...
public partial class MainWindow : Window
{
    ...
    {
        if (Box1.IsChecked.Value) grGrid.ColumnDefinitions[0].Width = new GridLength(1000, GridUnitType.Star);
        else grGrid.ColumnDefinitions[0].Width = new GridLength(0);
    }

    private void IsShortNameCheckBoxChecked(object? sender, RoutedEventArgs e)
    {
        if (Box2.IsChecked.Value) grGrid.ColumnDefinitions[1].Width = new GridLength(1000, GridUnitType.Star);
        else grGrid.ColumnDefinitions[1].Width = new GridLength(0);
    }
